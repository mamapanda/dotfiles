...
** Basic Keybindings
   ...
   The default ~s~ and ~S~ aren't that useful in my opinion, and ~s~ is slightly
   easier to press than ~evil-ex~'s default binding of ~:~.
...
*** Dollar
    #+BEGIN_SRC emacs-lisp
      (targets-define-to dollar "$" nil quote :bind t :keys "$")
    #+END_SRC
